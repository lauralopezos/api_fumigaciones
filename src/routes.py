...
consumidores_bp = Blueprint(
    "consumidores",
    __name__,
    url_prefix="/consumidores")
tecnicos_bp = Blueprint("tecnicos", __name__, url_prefix="/tecnicos")
administradores_bp = Blueprint(
    "administradores",
    __name__,
...
def crear_consumidor():
    ...
    return jsonify({"id": new_id, "nombre": nombre,
                   "email": email, "direccion": direccion}), 201


@consumidores_bp.get("")
def listar_consumidores():
    return jsonify(
        fetch_all("SELECT id, nombre, email, direccion FROM consumidores")), 200


@consumidores_bp.get("/<int:cid>")
def obtener_consumidor(cid):
    row = fetch_one(
        "SELECT id, nombre, email, direccion FROM consumidores WHERE id=%s", (cid,))
    if not row:
...
def actualizar_consumidor(cid):
    ...
    vals.append(cid)
    execute(
        f"UPDATE consumidores SET {
            ', '.join(campos)} WHERE id=%s",
        tuple(vals))
...
def crear_tecnico():
    ...
    return jsonify({"id": new_id, "nombre": nombre,
                   "telefono": telefono, "especialidad": especialidad}), 201


@tecnicos_bp.get("")
def listar_tecnicos():
    return jsonify(
        fetch_all("SELECT id, nombre, telefono, especialidad FROM tecnicos")), 200


@tecnicos_bp.get("/<int:tid>")
def obtener_tecnico(tid):
    row = fetch_one(
        "SELECT id, nombre, telefono, especialidad FROM tecnicos WHERE id=%s", (tid,))
    if not row:
...
def actualizar_tecnico(tid):
    ...
    if especialidad is not None:
        ...
        vals.append(especialidad)
    if not campos:
        return jsonify({"detail": "Nada para actualizar"}), 400
    vals.append(tid)
    execute(
        f"UPDATE tecnicos SET {
            ', '.join(campos)} WHERE id=%s",
        tuple(vals))
...
def crear_admin():
    ...
    if fetch_one("SELECT id FROM administradores WHERE email=%s", (email,)):
        return jsonify({"detail": "El email ya existe"}), 400
    _, new_id = execute(
        "INSERT INTO administradores (nombre, email) VALUES (%s,%s)", (nombre, email))
...
def listar_admins():
    return jsonify(
        fetch_all("SELECT id, nombre, email FROM administradores")), 200


@administradores_bp.get("/<int:aid>")
def obtener_admin(aid):
    row = fetch_one(
        "SELECT id, nombre, email FROM administradores WHERE id=%s", (aid,))
    if not row:
