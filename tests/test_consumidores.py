def test_crud_consumidores(client):
    ...
    # LIST
    r = client.get("/consumidores"); assert r.status_code == 200
    assert any(x["id"] == cid for x in r.get_json())

    # GET ONE
    r = client.get(f"/consumidores/{cid}"); assert r.status_code == 200
    ...
    # DELETE
    r = client.delete(f"/consumidores/{cid}"); assert r.status_code == 200

    # 404 after delete
    r = client.get(f"/consumidores/{cid}"); assert r.status_code == 404
